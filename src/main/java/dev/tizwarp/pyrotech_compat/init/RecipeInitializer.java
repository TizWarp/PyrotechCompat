    ...
    private static void registerInheritedRecipes(){
        if (ModuleTechBasicConfig.IRONCLAD_ANVIL.INHERIT_GRANITE_ANVIL_RECIPES){
            for (AnvilRecipe recipe : anvilRecipes){
                ModuleTechBasic.Registries.ANVIL_RECIPE.register(new AnvilRecipe(recipe.getOutput(), recipe.getInput(), Math.max(1, (int)((double)recipe.getHits() * Math.max(0.0D, ModuleTechBasicConfig.IRONCLAD_ANVIL.INHERITED_GRANITE_ANVIL_RECIPE_HIT_MODIFIER))), recipe.getType(), AnvilRecipe.EnumTier.IRONCLAD).setRegistryName(PyrotechCompat.MODID, "granite_anvil/" + recipe.getRegistryName().getPath()));
            }
        }
        if (ModPyrotech.INSTANCE.isModuleEnabled(ModuleTechBasic.class) && ModuleTechMachineConfig.MECHANICAL_COMPACTING_BIN.INHERIT_COMPACTING_BIN_RECIPES){
            for (CompactingBinRecipe recipe : compactingBinRecipes){
                ModuleTechMachine.Registries.MECHANICAL_COMPACTING_BIN_RECIPES.register(new MechanicalCompactingBinRecipe(recipe.getOutput(), recipe.getInput(), recipe.getAmount(), recipe.getRequiredToolUses()).setRegistryName(PyrotechCompat.MODID, "compacting_bin/" + recipe.getRegistryName().getPath()));
            }
        }
        if (ModPyrotech.INSTANCE.isModuleEnabled(ModuleTechBasic.class) && ModuleTechMachineConfig.STONE_KILN.INHERIT_PIT_KILN_RECIPES){
            for (KilnPitRecipe recipe : kilnPitRecipes){
                ...
                ModuleTechMachine.Registries.STONE_KILN_RECIPES.register(createStoneKilnRecipe(recipe.getOutput(), recipe.getInput(), timeTicks, failureChance, recipe.getFailureItems(), "pit_kiln/" + recipe.getRegistryName().getPath()));
            }
        }
        if (ModuleTechMachineConfig.BRICK_KILN.INHERIT_STONE_TIER_RECIPES){
            for (StoneKilnRecipe recipe : stoneKilnRecipes){
                ...
                ModuleTechMachine.Registries.BRICK_KILN_RECIPES.register(new BrickKilnRecipe(recipe.getOutput(), recipe.getInput(), timeTicks, failureChance, recipe.getFailureItems()).setRegistryName(PyrotechCompat.MODID,"stone_kiln/" + recipe.getRegistryName().getPath()));
            }
        }
    }
